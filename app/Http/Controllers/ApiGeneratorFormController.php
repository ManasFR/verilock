...
class ApiGeneratorFormController extends Controller
{
...
function handle_license_validation{$randomNumber}() {
    ...
    if (isset(\$result['status']) && \$result['status'] === 'success') {
        ...
        error_log("API Validation Failed: " . print_r(\$result, true)); // Logs the full API response
        error_log("API Response Code: " . wp_remote_retrieve_response_code(\$response)); // Logs the HTTP response code
        
        wp_send_json([
            'success' => false, 
            'message' => \$result['message'] ?? 'Unexpected response from API.',
            'debug' => [
                'response_code' => wp_remote_retrieve_response_code(\$response), // Adds HTTP status code to response
                'response_body' => \$result // Includes the raw API response for troubleshooting
            ]
        ]);
    }
...
EOD;
    ...
    public function index(){
        return view('admin.apigenerate.apigenerate');
    }
}